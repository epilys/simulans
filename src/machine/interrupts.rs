...
#[derive(Copy, Clone, Debug)]
pub struct InterruptConnectRequest {
    pub connect: bool,
    pub interrupt_id: u16,
    pub cpu_id: u8,
}
...
pub struct InterruptGenerator {
    ...
    pub connect_sender: Sender<InterruptConnectRequest>,
}

pub struct Interrupts {
    pub generator: InterruptGenerator,
    generator_rcv: Receiver<InterruptRequest>,
    connector_rcv: Receiver<InterruptConnectRequest>,
...
impl Interrupts {
    pub fn new() -> Self {
        let (irq_sender, generator_rcv) = sync_channel(64);
        let (connect_sender, connector_rcv) = channel();
        let generator = InterruptGenerator {
            irq_sender,
            connect_sender,
        };
    ...
    pub fn rcv(&self, ignore: bool) -> bool {
        ...
        if let Ok(irq) = self.generator_rcv.try_recv() {
            if !ignore {

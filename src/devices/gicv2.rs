...
/// Return `true` if the first argument of the function has a higher priority than the second argument.
...
pub struct Gicd {
    ...
    pub igroupr0: [u32; 8],
    /// Enabled interrupts, set and cleared by writing to Interrupt Set-Enable Registers & Interrupt Clear-Enable Registers.
    pub enabled_interrupts: [u32; 32],
    /// Pending interrupts, set and cleared by writing to Interrupt Set-Pending Registers & Interrupt Clear-Pending Registers.
    pub pending_interrupts: [u32; 32],
    /// Active interrupts, set and cleared by writing to Interrupt Set-Active Registers & Interrupt Clear-Active Registers.
...
impl Gicd {
    ...
    /// Returns `true` if the field in the `GICD_IGROUPRn` register associated with the argument
    /// `interrupt_id` is set to 0, indicating that the interrupt is configured as a Group 0
    /// interrupt.
    #[doc(alias("IsGrp0Int"))]
    ...
    /// Returns an 8-bit field specifying which CPUs should receive the interrupt specified by argument
    /// `interrupt_id`
...
impl Gicc {
    ...
    fn read_iar(&self, gicd: &mut Gicd, cpu_id: u8) -> u32 {
        ...
        {
            pend_id = SPURIOUS_INTERRUPT; // If the highest priority isn't enabled, then no interrupt
        }
        ...
        let sgi_id = if pend_id < 16 {
            ...
            unimplemented!() // SGI_CpuID(pend_id) // value is IMPLEMENTATION DEFINED
        } else {

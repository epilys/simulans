...
/// Most SCSI commands are just identified by the opcode - the first byte of the
/// CDB - but some opcodes require a second byte, called the service action.
/// Generally, each distinct service action value is treated as a first-class
/// command. But there's some weirdness around parsing, especially with invalid
/// commands: sometimes, we're expected to behave differently for a valid opcode
/// with an invalid service action vs an invalid opcode.
///
/// To allow for this, we have a two-step parsing API.
/// First, a caller calls `parse_opcode` with the first byte of the CDB.
/// This could return three things:
///

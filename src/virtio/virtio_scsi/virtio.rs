...
impl Request {
    pub fn parse(reader: &mut impl Read) -> Result<Self, RequestParseError> {
        ...
        let lun = VirtioScsiLun::parse(request[0..8].try_into().expect("slice is of length 8"))
            .ok_or_else(|| {
                RequestParseError::FailedParsingLun(
...
impl Response {
    pub fn write(&self, writer: &mut impl Write) -> Result<(), io::Error> {
        ...
        writer.write_all(&[self.code as u8])?; // response
...
// virtio_scsi_cmd_resp};     use virtio_queue::{desc::RawDescriptor,
// mock::MockSplitQueue};     use vm_memory::{ByteValued, GuestAddress,
// GuestMemoryMmap};
...
//             GuestMemoryMmap::from_ranges(&[(GuestAddress(0),
// 0x1000_0000)]).unwrap();

//         // The `build_desc_chain` function will populate the `NEXT` related
// flags and         // field.
//         let v = vec![
...
//         let req = Request::parse(&mut chain).expect("request failed to
// parse");         assert_eq!(req.lun, VirtioScsiLun::ReportLuns);
//     }
// }
